...
这就是 **EventLoop + Client** 模式——网络层和业务层分离，各司其职。
...
:::note[自定义编解码器]
内置编解码器能满足大多数场景。如果你需要更精细的控制——比如自定义消息格式、压缩、加密——可以实现 `Codec` trait。我们会在[自定义协议](/02-protocols/10-custom-protocol)章节详细介绍如何从零实现编解码器。
:::
