...
:::tip[配套工具]
本教程配套的桌面应用展示了连接的安全状态。你可以观察 Noise 握手完成后，连接从"未加密"变为"已加密"的过程。
:::
...
1. **配置 Noise 加密**
   ...
   `noise::Config::new` 是一个函数，接收 keypair 并返回配置。SwarmBuilder 会自动将身份密钥传递给它。
