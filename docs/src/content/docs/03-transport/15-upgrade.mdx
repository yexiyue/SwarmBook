...
:::tip[配套工具]
本教程配套的桌面应用可以显示每个连接使用的协议栈。你可以观察安全层、复用层、应用协议的协商过程。
:::

<Steps>

1. **理解 SwarmBuilder 的配置**
...
3. **查看连接升级日志**
   ...
   ```rust
   std::env::set_var("RUST_LOG", "libp2p_core::upgrade=debug");
...
sequenceDiagram
    ...
    Note over A,B: 安全层协商 (Noise/TLS)
    Note over A,B: Noise XX 握手
    Note over A,B: 复用层协商 (Yamux)
    Note over A,B: 应用协议协商
```

</TabItem>
<TabItem label="QUIC 协商流程">
...
| 章节 | 内容 |
|-----|------|
| 11 | 传输层抽象与 Transport trait |
| 12 | TCP 与 QUIC 对比 |
| 13 | Noise 加密握手 |
| 14 | Yamux 流复用 |
| 15 | 协议协商与连接升级 |

有了传输层的基础，下一篇我们将进入**节点发现**——学习 libp2p 如何在 P2P 网络中发现和连接其他节点。
